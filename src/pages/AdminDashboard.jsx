...
const MOCK_REPORTS = [
  { id: 'r1', category: 'harassment', details: 'Sending inappropriate messages', status: 'pending', createdAt: new Date(Date.now() - 86400000) },
  { id: 'r2', category: 'spam', details: 'Promoting external links', status: 'resolved', createdAt: new Date(Date.now() - 172800000) },
];

const MOCK_ADMIN_USERS = [
  { id: 'u1', displayName: 'Alex Rivera', username: 'alex_vibes', email: 'alex@example.com', isPremium: true, isOnline: true, photoURL: '', isSuspended: false },
  { id: 'u2', displayName: 'Priya Sharma', username: 'priya_chill', email: 'priya@example.com', isPremium: false, isOnline: true, photoURL: '', isSuspended: false },
  { id: 'u3', displayName: 'Sam Chen', username: 'sam_gaming', email: 'sam@example.com', isPremium: false, isOnline: false, photoURL: '', isSuspended: true },
];
...
const AdminDashboard = () => {
  ...
  const handleReportAction = (reportId, action) => {
    setReports(prev => prev.map(r => r.id === reportId ? { ...r, status: action, resolvedAt: new Date() } : r));
  };
    ...
    <div style={{ padding: 'var(--space-xl)', maxWidth: '1100px', margin: '0 auto' }} className="animate-fadeIn">
      ...
      {loading ? (
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: 'var(--space-lg)', borderBottom: '1px solid var(--border-light)', fontWeight: '600' }}>Reports ({reports.length})</div>
          {reports.length === 0 ? (
            ...
            reports.map(report => (
              <div key={report.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', padding: 'var(--space-md) var(--space-lg)', borderBottom: '1px solid var(--border-light)' }}>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.9rem', fontWeight: '600' }}>{REPORT_CATEGORIES.find(c => c.id === report.category)?.label || report.category}</div>
                ...
                <span className={`badge ${report.status === 'pending' ? 'badge-warning' : report.status === 'resolved' ? 'badge-success' : 'badge-danger'}`}>{report.status}</span>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: 'var(--space-lg)', borderBottom: '1px solid var(--border-light)', fontWeight: '600' }}>Users ({users.length})</div>
          {users.map(user => (
            <div key={user.id} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', padding: 'var(--space-md) var(--space-lg)', borderBottom: '1px solid var(--border-light)' }}>
              ...
              <div className={`badge ${user.isOnline ? 'badge-success' : 'badge-primary'}`} style={{ fontSize: '0.7rem' }}>{user.isOnline ? 'Online' : 'Offline'}</div>
              <div style={{ display: 'flex', gap: '4px' }}>
                <button className="btn btn-sm btn-ghost" title="View"><HiOutlineEye /></button>
                {!user.isSuspended ? (
                  <button className="btn btn-sm btn-ghost" style={{ color: 'var(--accent-orange)' }} onClick={() => handleUserAction(user.id, 'suspend')} title="Suspend"><HiOutlineBan /></button>
                ) : (
                  <button className="btn btn-sm btn-ghost" style={{ color: 'var(--accent-green)' }} onClick={() => handleUserAction(user.id, 'unsuspend')} title="Unsuspend"><HiOutlineCheck /></button>
                )}
              </div>
            </div>
          ))}
